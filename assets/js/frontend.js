(function () {
    ...
    function handleClipboardFallback(prompt, platform, button) { var fallbackMessage = 'Prompt copied — Paste into your AI assistant'; var fallbackError = 'Could not copy prompt — Please copy manually'; if (!prompt) { setInlineStatus(button, fallbackError, true); return; } function markFallbackSuccess() { emitAnalytics('ai_share_fallback_copy', { ai_platform: platform, page_url: window.location.href }); setInlineStatus(button, fallbackMessage, false);} function markFallbackFailure() { setInlineStatus(button, fallbackError, true);} if (navigator.clipboard && window.isSecureContext) { navigator.clipboard.writeText(prompt).then(markFallbackSuccess).catch(function () { if (copyWithExecCommand(prompt)) { markFallbackSuccess(); return; } markFallbackFailure(); }); return; } if (copyWithExecCommand(prompt)) { markFallbackSuccess(); return; } markFallbackFailure(); }
    ...
    function getPromptContext(button) { return { template: button.dataset.template || '', platform: button.dataset.ai, title: button.dataset.title || document.title || '', site: button.dataset.site || window.location.hostname || '', url: button.dataset.url || getCanonicalUrl() }; }
    ...
    function onShareButtonClick(event) { event.preventDefault(); var button = event.currentTarget; var context = getPromptContext(button); var prompt = applyPromptTemplate(context.template, { url: context.url, site: context.site, title: context.title }); var promptFingerprint = getPromptFingerprint(context.platform, prompt); if (handleThrottle(button, promptFingerprint)) return false; openProviderOrFallback(context.platform, prompt, button); emitAnalytics('ai_share_click', { ai_platform: context.platform, page_url: window.location.href }); return false; }
