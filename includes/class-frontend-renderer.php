...
class AI_Share_Links_Frontend_Renderer {
    ...
    private function generate_share_buttons($options) {
        ...
        foreach ($options['enabled_ais'] as $ai_key) {
            ...
            $output .= sprintf('<a href="%s" target="_blank" rel="noopener noreferrer" class="ai-share-btn" data-ai="%s" data-template="%s" data-site="%s" data-url="%s" data-title="%s" data-type="%s" data-post-type="%s" data-category="%s" data-tags="%s" data-excerpt="%s" data-schema-type="%s" data-author="%s" data-published-date="%s" data-faq-count="%s" data-base-url="%s" data-param-key="%s" data-supports-prefill="%s" data-prefill-stability="%s" data-recommended-mode="%s"%s>%s<span>%s</span></a>', esc_url($fallback_url), esc_attr($ai['id']), esc_attr($options['ai_prompt']), esc_attr($site_name), esc_attr($post_url), esc_attr($page_title), esc_attr($prompt_context['type']), esc_attr($prompt_context['post_type']), esc_attr($prompt_context['category']), esc_attr($prompt_context['tags']), esc_attr($prompt_context['excerpt']), esc_attr($prompt_context['schema_type']), esc_attr($prompt_context['author']), esc_attr($prompt_context['published_date']), esc_attr((string) $prompt_context['faq_count']), esc_url($ai['base_url']), esc_attr($ai['param_key']), $ai['supports_prefill'] ? '1' : '0', esc_attr($ai['prefill_stability']), esc_attr($ai['recommended_mode']), $onclick, $icon, esc_html($button_text));
        }
    ...
    public function get_provider_map() {
        return array(
            'perplexity' => array('id' => 'perplexity', 'label' => 'Perplexity', 'icon' => 'P', 'base_url' => 'https://www.perplexity.ai/search', 'param_key' => 'q', 'supports_prefill' => true, 'prefill_stability' => 'stable', 'recommended_mode' => 'prefill', 'mode_description' => __('Opens Perplexity with your prompt prefilled and also keeps clipboard fallback for blocked popups.', AI_SHARE_LINKS_TEXT_DOMAIN), 'enabled' => true),
            'chatgpt' => array('id' => 'chatgpt', 'label' => 'ChatGPT', 'icon' => 'C', 'base_url' => 'https://chat.openai.com/', 'param_key' => 'q', 'supports_prefill' => true, 'prefill_stability' => 'experimental', 'recommended_mode' => 'auto', 'mode_description' => __('Tries to open ChatGPT with a prefilled prompt when possible, then falls back to copied prompt messaging.', AI_SHARE_LINKS_TEXT_DOMAIN), 'enabled' => true),
            'claude' => array('id' => 'claude', 'label' => 'Claude', 'icon' => 'C', 'base_url' => 'https://claude.ai/new', 'param_key' => 'prompt', 'supports_prefill' => true, 'prefill_stability' => 'experimental', 'recommended_mode' => 'auto', 'mode_description' => __('Uses automatic mode: attempts prefill first and falls back to copy-only guidance when reliability is limited.', AI_SHARE_LINKS_TEXT_DOMAIN), 'enabled' => true),
            'gemini' => array('id' => 'gemini', 'label' => 'Gemini', 'icon' => 'G', 'base_url' => 'https://gemini.google.com/app', 'param_key' => 'prompt', 'supports_prefill' => true, 'prefill_stability' => 'experimental', 'recommended_mode' => 'auto', 'mode_description' => __('Uses automatic mode: attempts prefill first and falls back to copy-only guidance when reliability is limited.', AI_SHARE_LINKS_TEXT_DOMAIN), 'enabled' => true),
            'deepseek' => array('id' => 'deepseek', 'label' => 'DeepSeek', 'icon' => 'D', 'base_url' => 'https://chat.deepseek.com/', 'param_key' => 'q', 'supports_prefill' => false, 'prefill_stability' => 'unsupported', 'recommended_mode' => 'copy_only', 'mode_description' => __('Copies the prompt and opens DeepSeek without relying on URL prefill parameters.', AI_SHARE_LINKS_TEXT_DOMAIN), 'enabled' => true),
        );
    }
